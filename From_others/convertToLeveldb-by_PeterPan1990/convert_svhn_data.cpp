...
const int kCIFARTrainBatchSize = 73257;
const int kCIFARTestBatchSize = 26032;
...
void convert_dataset(const string& input_folder, const string& output_folder) {
  ...
  status = leveldb::DB::Open(options, output_folder + "/svhn_train_leveldb",
      &train_db);
  CHECK(status.ok()) << "Failed to open leveldb.";
  for (int fileid = 0; fileid < kCIFARTrainBatches; ++fileid) {
    // Open files
    LOG(INFO) << "Training Batch " << fileid + 1;
    snprintf(str_buffer, kCIFARImageNBytes, "/train_batch.bin");
    ...
    for (int itemid = 0; itemid < kCIFARTrainBatchSize; ++itemid) {
      ...
      snprintf(str_buffer, kCIFARImageNBytes, "%05d",
          fileid * kCIFARTrainBatchSize + itemid);
  ...
  leveldb::DB* test_db;
  CHECK(leveldb::DB::Open(options, output_folder + "/svhn_test_leveldb",
